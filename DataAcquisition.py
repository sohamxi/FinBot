        ...
        return Task(
            description=dedent(f"""\
                ...
                - Store the results in the shared repository:
                  Order Book Path: /data/raw/{market.lower()}/{asset_class.lower()}/orderbook/{instrument_name}_{TIMESTAMP}.json
        ...
        return Task(
            description=dedent(f"""\
                ...
                - Store output in shared directories:
                  News: /data/raw/{market.lower()}/{asset_class.lower()}/news/{instrument_name}_{SOURCE}_{DATE}.json
